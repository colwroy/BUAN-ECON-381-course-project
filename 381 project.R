...
#make predictions and use them to plot a regression line on a scatter plot
housing2_training$pred <- predict(priceVarea, housing2_training)
ggplot(housing2_training, aes(x = area, y = price)) + geom_point() + geom_line(aes(y = pred), color = "purple")
...
priceVarea_LogLog <- lm(price~log(area), data = housing2_training)
housing2_training$pred_LogLog <- predict(priceVarea_LogLog, housing2_training)
ggplot(housing2_training, aes(x = area, y = price)) + geom_point() + geom_line(aes(y = pred_LogLog), color = "purple")

summary(priceVarea_LogLog)
