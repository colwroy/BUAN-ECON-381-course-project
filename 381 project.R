...
housing1 <- housing1[housing1$price < 11000, ]
housing1 <- housing1[housing1$area < 14000, ]
...
spline_priceVarea <- gam(price ~ s(area), data = housing2_testing, family = 'gaussian')
...
###########################################################
#SPLINE has the lowest RMSE (~1421) on the validation set
##############################################################

#spline_priceVarea <- gam(price ~ s(area), data = housing2_testing, family = 'gaussian')

housing2_testing$spline_priceVarea_predONtest <- predict(spline_priceVarea, housing2_testing)

(spline_priceVarea_RMSEtest <- sqrt(mean((housing2_testing$price-housing2_testing$spline_priceVarea_predONtest)^2)))
...
svm_tuned_priceVarea_RMSE <- c("svm_tuned",svm_tuned_RMSEtrain, svm_tuned_RMSEvalidation)
...
#errors on TUNED model
housing2_training$boost_tune_predONtrain <- predict(tuned_tree, housing2_training)
housing2_validation$boost_tune_predONvalidation <- predict(tuned_tree, housing2_validation)
...
model <- data.frame(matrix(ncol = 3, nrow = 1))
colnames(model) <- c("Model_Name","RMSEtrain", "RMSEvalidation")
model[1, ] <- basemulti_RMSE
model[nrow(model) + 1, ] <- ridge_RMSE
model[nrow(model) + 1, ] <- svm_RMSE
model[nrow(model) + 1, ] <- svm_tuned_priceVarea_RMSE
model[nrow(model) + 1, ] <- CART_RMSE
model[nrow(model) + 1, ] <- tunedCART_RMSE
model[nrow(model) + 1, ] <- gradboost_RMSE
model[nrow(model) + 1, ] <- boost_tune_RMSE

############################################################
#base multivariate model performs the best (RMSE = 1124), 
#followed by ridge regularized version (RMSE = 1127)
#on validation set
#############################################################
...
probit_bin <- glm(acbinary~price+stories,data=housing2_testing, family = binomial(link = "probit"))
...
##################################################################
#logit binary model performs the highest 
#at an accuracy of 0.753 on the validation data partition
#####################################################################

#logistic classification model

#logit_bin <- glm(acbinary~price+stories, data=housing2_testing, family = "binomial")

housing2_testing$logit_bin_predONtest <- predict(logit_bin, housing2_testing, type = "response")

roc_obj_logitest <- roc(housing2_testing$acbinary, housing2_testing$logit_bin_predONtest)

test_logi_auc <- auc(roc_obj_logitest)

plot(roc_obj_logitest, main="ROC Curve test logi", col="#1c61b1", lwd=4, print.auc=TRUE)

(logit_bin_accuracyTEST<-mean((housing2_testing$logit_bin_predONtest>0.7)==housing2_testing$acbinary))
